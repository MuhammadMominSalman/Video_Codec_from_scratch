...
load(coded_file, 'width', 'height', 'transform_blocksize', 'qp', 'huffman_table', 'huffman_table_mv', 'resd', 'presd', 'bitstream', 'bitstream2', 'motion_vectors_allframes', 'me_blocksize', 'me_searchrange')
...
if intra == 0
    for i = 1:size(runlevel_representation,1)
        ...
        if count == num_blocks_per_frame
            ...
            zigzag_scanned = blockbased_deadzone_dequantizer_from_levels(input_levels, quantisation_matrix);
...
if intra == 1
    for i = start_index:size(runlevel_representation,1)
        ...
        if count == num_blocks_per_frame
            ...
            zigzag_scanned = blockbased_deadzone_dequantizer_from_levels(input_levels, quantisation_matrix);
            itransformed_image = blockbased_idct_on_image(zigzag_scanned, transform_blocksize);
            x = sum(sum(itransformed_image - resd));
            y = sum(sum(predicted_frame - presd));
